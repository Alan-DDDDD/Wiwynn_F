...
$(`#DetailList`).on(`click`,'tr',function(){
  ...
  $(`#CTYPE option`).removeAttr("selected").filter(`[value=${tds[2].innerHTML}]`).attr("selected",true);
  $(`#CDDL option`).removeAttr("selected").filter(`[value=${tds[3].innerHTML}]`).attr("selected",true);
  $(`#REFVALTYPE option`).removeAttr("selected").filter(`[value=${tds[4].innerHTML || ""}]`).attr("selected",true);
